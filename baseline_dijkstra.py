...
# Fucntion that finds index of the value in a two-level list
...
def calculate_subjective_weight_for_edge_time(graph, agent, time, 
                                              edge, iteration, statistics):
    """ Function that finds given agent's subjective weight 
    for the given edge at given arrival time in given iteration.

    The summary of the process:

    1) Find whether agent traversed given edge at given time in given
       iteration. If not, return the real weight of edge.
    2) Get agent's experience regarding the traversal and
       return a subjective representation of edge's weight
       in the eyes of the agent.
    """
    
    real_weight = graph[edge[0]][edge[1]]['weight']
    t = time
    indexes_edge = find_index_2_levels(
                    statistics[iteration][agent.id_num].edges_passed, 
                    edge)

    # If agent never was on the edge
    if not indexes_edge:
    ...
    if indexes_edge:
        for idx_edge in indexes_edge:
            time_interval_a = statistics[iteration][agent.id_num].\
            real_edge_times[idx_edge[0]][idx_edge[1]][0]
            
            time_interval_b = statistics[iteration][agent.id_num].\
            real_edge_times[idx_edge[0]][idx_edge[1]][1]
            ...
            if time >= time_interval_a and time <= time_interval_b:
                ...
                # Calculate objective and subjective weights agent 
                ...
                for tp in range(1, len(time_preserver)):     
                    ...
                    subjective_weight += ((time_preserver[tp] 
                                          - time_preserver[tp-1])
                                          * 60)
            ...
            if (time < time_interval_a
                ...
                # edge by the time of his first memories.
                ...
                # Calculate fresh objective and subjective 
...
# Function that finds average subjective weight of 
# the edge-time pair given all previous rides.
def calculate_subjective_weight_for_edge_time_overall(graph, agent, time, 
                                                      edge, statistics):
    subjective_weights_all_iters = []
    discount = []
    for i in range(agent.current_iter + 1):
        subjective_weight = calculate_subjective_weight_for_edge_time(
                            graph=graph, agent=agent, time=time, 
                            edge=edge, iteration=i, statistics=statistics)
        if subjective_weight[1]:
            subjective_weights_all_iters.append(subjective_weight[0])

    # The final weight across all iterations is calculated by 
    # averaging all the estimates from previous iterations 
    # of simulation if they were really experienced.
    if not subjective_weights_all_iters:
...
# Function that finds the shortest path from 
# source to target beginning in time = t.
def dijkstra_with_time_dependent_weights(graph, source, target, agent, 
                                         time, statistics, length=False):
    ...
    while dist_proxy:
        ...
        for vertex in adj_list[min_node]:
            vertex = vertex - 1
            cost = calculate_subjective_weight_for_edge_time_overall(
                    graph=graph, 
                    agent=agent,\
                    time=time, 
