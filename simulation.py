...
class Simulation(object):
    ...
    # Function that counts how speed on the road 
    # is affected by the number of cars on it.
    ...
    # Function that calculates the distance agent 
    # passed on the current edge by current time.
    def calculate_passed_distance(self, agent, current_time):
        ...
        for time, speed in agent.speed_by_time.items():
            if (time >= agent.real_edge_times[agent.current_path]\
                       [agent.current_edge_num][0] 
                and time <= current_time):
                time_passed_on_edge.append(time - agent.real_edge_times\
                                [agent.current_path][agent.current_edge_num][0])
                speed_preserver.append(speed)
                current_part_passed_time = (time_passed_on_edge[-1] 
                                           - time_passed_on_edge[-2])
                passed_distance += (current_part_passed_time 
                                   * speed_preserver[-2])
    ...
    def calculate_real_edge_time_for_edge(self, agent, path_num, edge_num):
        ...
        if edge_num == 0:
            # Real edge time is calculated at first by dividing
            # objective weight of the edge by agent's real speed at
            # the moment he arrived on it, e.g. agent makes a forecast
            # in which time he will finish the edge with current speed.
            ...
            list((time, time + (self.graph[edge[0]][edge[1]]['weight'] 
                / agent.speed)))
        ...
        for fl in range(2):
            ...
            float(("{0:." + str(digits_to_save) + "f}").format(round(
                agent.real_edge_times[path_num][edge_num][fl], digits_to_save)))
    ...
    # ones (uses function agent.restore() from class Agent).
    ...
    # maximim time for the simulation in order for all drivers to 
    # finish their trips.
            ...
            for path_num in range(len(a.paths)):
                if a.paths[path_num] is None:
                    ...
                    for edge_num in range(len(a.paths[path_num]) - 1):
                        if (a.real_edge_times[path_num][edge_num][-1] is not None
...
class Simulation_baseline(object):
    ...
    def calculate_expected_edge_time_for_edge(self, agent, path_num, edge_num):
        digits_to_save = len(str(agent.time_stamp)) - 2
        if edge_num == 0:
            # Expected edge time is calculated as the edge subjective 
            # (for agent) weight divided by 60 kmh (default speed). 
            # Expected edge time is a time in which agent expects to 
            # finish traversing the edge.
            time = agent.times_of_departures[path_num]
            edge = (agent.paths[path_num][edge_num], 
                    agent.paths[path_num][edge_num+1])
            ...
            / agent.speed))))
            # Real edge times modified below to copy the expected 
            # edge times without reference.
            ...
            agent.expected_edge_times[path_num].append(list((time, time
             + (calculate_subjective_weight_for_edge_time_overall(
                                                ...
                                                statistics=self.common_sense)
             / agent.speed))))
            # Real edge times modified below to copy the 
            # expected edge times without reference.
        ...
        for fl in range(2):
            ...
            float(("{0:." + str(digits_to_save) + "f}").format(round(
                agent.expected_edge_times[path_num][edge_num][fl], digits_to_save)))
        agent.real_edge_times[0][0][0] = \
        float(("{0:." + str(digits_to_save) + "f}").format(round(
            agent.real_edge_times[0][0][0], digits_to_save)))
    ...
    def simulation_iteration(self):
        ...
        # Priorty qeues for every edge and counters for each edge to 
        # check whether every agent who came to edge also left it.
        ...
        while arch_time <= int((self.max_time/self.agents[0].time_stamp)):
            ...
            t = float(("{0:." + str(len(str(self.agents[0].time_stamp))-2) 
                + "f}").format(round((0 + arch_time*self.agents[0].time_stamp), 
                                      digits_to_save)))
                    ...
                    if a.paths[path_num] is not None:
                        for edge_num in range(len(a.paths[path_num])-1):
                            ...
                            if a.real_edge_times[path_num][edge_num][1] == t:
                                ...
                                # Find the edge from which agent leaves
                                # and decrease the number of cars on it
                                # by one. Also, dequeue agent from this edge.
                                ...
                                #print('number_of_cars_after', 
                                      #len(agents_on_edges[edge_name]))
                                #print('time',t)
                                #print('edge',edge_name)

                                # Cleans current edge variables and 
                                # transfers time of edge-finish to next
                                # edge start time. Or in case this is 
                                # the end the path for agent increases 
                                # his current path number by one.
                                a.current_edge = (0, 0)
                                a.current_edge_num = []
                                ...
                                # leaving at the current time) from 
                                # which another agent left.
                                # Their speed must change due to decrese
                                # in number of cars ahead of them.
                                cntr = -1
                                for agent in agents_on_edges[edge_name]:
                                    cntr += 1
                                    if (agent.current_edge == edge_name and 
                                        (agent.real_edge_times[agent.current_path]\
                                        [agent.current_edge_num][1] != t)):

                                        # Searching for the position
                                        # of the agent on an edge and finding
                                        # number of cars ahead.
                                        num_of_cars_before_agent_on_an_edge = \
                                        len(agents_on_edges[edge_name][:int(cntr)])

                                        # Recalculating speed and saving
                                        # it in an ordered dict.
                                        agent.speed = self.speed_revisited(
                                            num_of_cars_before_agent_on_an_edge, 
                                            agent)
                                        agent.speed_by_time[t] = int(agent.speed)

                                        # Calculating the distance on
                                        # edge which is already passed
                                        # by an agent, distance to pass,
                                        # and finally renovating the 
                                        # real travel time on the edge.
                                        passed_distance = \
                                        self.calculate_passed_distance(
                                                                agent=agent, 
                                                                current_time=t)
                                        weight_of_the_edge = \
                                        ...
                                        #print('Time of arrival before change', 
                                              #agent.real_edge_times[agent.current_path]\
                                              #[agent.current_edge_num][1])
                                        #print('Passed Distance', passed_distance)

                                        time_to_pass = float(("{0:." + str(digits_to_save) 
                                            + "f}").format(round(distance_to_pass 
                                                                 / agent.speed, 2)))
                                        # Small fix to secure problems 
                                        # with rounding.
                                        if time_to_pass == 0.0:
                                            time_to_pass = agent.time_stamp
                                        agent.real_edge_times\
                                        [agent.current_path][agent.current_edge_num][1] = \
                                        float(("{0:." + str(digits_to_save) + "f}").format(
                                            round(t + time_to_pass, digits_to_save)))

                                        #Checking prints continued
                                        #print('Time of arrival after change', 
                                              #agent.real_edge_times[agent.current_path]\
                                              #[agent.current_edge_num][1])
            ...
            # Agent finds path and expects to traverse it 
            # for some time Arch-loop.
            for a in self.agents:
                for path_num in range(a.current_path, len(a.paths)):
                    ...
                    if (a.times_of_departures[path_num] == t and
                        ...
                        a.paths[path_num] = dijkstra_with_time_dependent_weights(
                                            graph=self.graph, 
                        ...
                        # To maintain expectation correctness it is 
                        # needed to increase agent's speed to 60 kmh.
                        a.speed = 60
                        # Then he finds the expected time he will be 
                        # on each edge, also, expected travel time.
                        for edge_num in range(len(a.paths[path_num])-1):
                            self.calculate_expected_edge_time_for_edge(
                                                            agent=a,
                                                            path_num=path_num, 
                                                            edge_num=edge_num)
                    ...
                    if a.paths[path_num] is not None:
                        for edge_num in range(len(a.paths[path_num])-1):
                            ...
                            if (a.real_edge_times[path_num][edge_num][0] == t and
                                ...
                                # Find the edge on which agent comes 
                                # and increase the number of cars on 
                                # it by one.
                                ...
                                # Increase the daily number of
                                # cars came to the edge.
                                edge_name_came[edge_name] += 1

                                # Recalculating speed and saving it in
                                # an ordered dict, also assigning 
                                # values to agent's current variables.
                                a.current_edge = edge_name
                                a.current_edge_num = edge_num
                                a.current_path = path_num
                                a.speed = self.speed_revisited(
                                            num_of_cars_before_agent_on_an_edge, 
                                            agent=a)
                                ...
                                #print('----------COMING----------')
                                #print('agent id', a.id_num)
                                #print('number of cars before', 
                                       #num_of_cars_before_agent_on_an_edge)
                                #print('time',t)
                                #print('speed', a.speed)
                                #print('edge', a.current_edge)

                                # Agent estimates his time of arrival
                                # to the end of the edge taking into
                                # account current speed on edge for him.
                                self.calculate_real_edge_time_for_edge(
                                                            agent=a, 
                                                            path_num=path_num, 
                                                            edge_num=edge_num)

                            # Postponing departure times for next trips
                            # if they are earlier then expected time of
                            # current path finish.
                            if (path_num <= len(a.paths)-2 and 
                                a.real_edge_times[path_num][edge_num][-1] is not None):
                                for i in range(1, len(a.paths)-path_num):
    ...
    # Function that copies agents and saves statistics about them.
    def copy_agents(self):
        copied_agents = []
        for a in self.agents:
            copied_agents.append(deepcopy(a))
    ...
    # Main function that repeatedly makes iterations of the simulation
    # to reach the common-sence equilibrium with shortest-distance paths.
    ...
    # Function that counts the number of paths that were
    # changed from previous to current iteration
    ...
    # Function that returns data from specified iteration's 
    # congestion on each edge at each time period specified
    # by time_stamp in Agent class initialization.
...
class Simulation_navigator(simulation_baseline):
    ...
    def calculate_expected_edge_time_for_edge(self, agent, path_num, edge_num):
        ...
        if edge_num == 0:
            ...
            if agent.navigator_user:
                ...
                edge = (agent.paths[path_num][edge_num], 
                        agent.paths[path_num][edge_num+1])
                agent.expected_edge_times[path_num].append(list((time, time
                    + (self.graph[edge[0]][edge[1]]['navigator_weight']
                        / agent.speed))))
            else:
                time = agent.times_of_departures[path_num]
        ...
        # Everything is rounded to maintain discrete time consistency.
        for fl in range(2):
            agent.expected_edge_times[path_num][edge_num][fl] = float(
                ("{0:." + str(digits_to_save) + "f}").format(
                    round(agent.expected_edge_times[path_num][edge_num][fl], 
                          digits_to_save)))
        agent.real_edge_times[0][0][0] = float(
            ("{0:." + str(digits_to_save) + "f}").format(
                round(agent.real_edge_times[0][0][0], digits_to_save)))
                                ...
                                for agent in agents_on_edges[edge_name]:
                                    ...
                                    if (agent.current_edge == edge_name and 
                                        ...
                                        agent.speed = self.speed_revisited(
                                            ...
                                            agent)
                                        agent.speed_by_time[t] = int(agent.speed)
                                        self.cars_ahead[edge_name].append(
                                            [agent.speed, 
                                            num_of_cars_before_agent_on_an_edge])
        ...
        # real equolibrium where not all drivers use navigator.
            ...
            for iteration in range(self.number_of_iterations):
                ...
                if iteration == self.number_of_iterations-1:
                    #self.statistics[0] = self.copy_agents()
                    self.statistics[0] = \
                    deepcopy(self.common_sense[len(self.common_sense) - 1])
                self.restore_all_agents()
                self.paths_changed_from_last_iter()
    ...
    # Function that counts the number of paths that were changed from
...
class Simulation_PTA(simulation_baseline):
    ...
    # Function that transforms initial historical data about
    ...
    def calculate_expected_edge_time_for_edge(self, agent, path_num, edge_num):
        ...
        if edge_num == 0:
            ...
            if agent.PTA_user:
                ...
                edge = (agent.paths[path_num][edge_num], 
                        agent.paths[path_num][edge_num+1])
                time_range = (self.calculate_weight_for_edge_time_diff(
                                                            arrival_time=time, 
                                                            edge=edge) 
                    / agent.speed)
                time_range = float(("{0:." + str(digits_to_save) + "f}").format(
                    round(time_range, digits_to_save)))
        ...
        for fl in range(2):
            ...
            float(("{0:." + str(digits_to_save) + "f}").format(round(
                agent.expected_edge_times[path_num][edge_num][fl], digits_to_save)))
        agent.real_edge_times[0][0][0] = float(
            ("{0:." + str(digits_to_save) + "f}").format(round(
                agent.real_edge_times[0][0][0], digits_to_save)))

    # Function that connects number of cars to speed with AdaBoost.
    def get_relationship_cars_ahead_speed(self):
        ...
        for edge, data in data_sets.items():
            ...
            # Build predictive model cars_ahead->speed.
    ...
    def dijkstra_PTA(self, source, target, time, length=False):
        ...
        while dist_proxy:
            ...
            for vertex in adj_list[min_node]:
                vertex = vertex - 1
                cost = self.calculate_weight_for_edge_time_diff(
                                                    arrival_time=time, 
            ...
            for a in self.agents:
                for path_num in range(a.current_path, len(a.paths)):
                    ...
                    if (a.times_of_departures[path_num] == t and 
                        ...
                        # To maintain expectation correctness we need
                        # to increase agent's speed to 60 kmh.
                        a.speed = 60
                        # Then he finds the expected time he will be on
                        # each edge, also, expected travel time.
                        for edge_num in range(len(a.paths[path_num])-1):
                            self.calculate_expected_edge_time_for_edge(
                                                            agent=a,
                                                            path_num=path_num, 
                        ...
                        if path_num <= len(a.paths)-2:
                            for i in range(1,len(a.paths)-path_num):
                                if (a.expected_edge_times[path_num][-1][-1] 
                                    ...
                                    float(("{0:." + str(len(str(self.agents[0].time_stamp)) - 2) 
                                        + "f}").format(round(
                                            a.times_of_departures[path_num + i], digits_to_save)))
...
class Dynamic_system(simulation_baseline):
    ...
    def choose_type_of_path(self, agent, path_num, t):
        ...
        if (min_path_len == baseline_path_len
            ...
            path = dijkstra_with_time_dependent_weights(
                                           graph=self.graph, 
                                           source=agent.start_nodes[path_num],
                                           target=agent.finish_nodes[path_num],
                                           agent=agent, 
        ...
        # Baseline and navigator are best ones.
            ...
            and min_path_len == navigator_path_len
            and min_path_len != PTA_path_len):
            baseline_path = dijkstra_with_time_dependent_weights(
                                            graph=self.graph, 
                                            source=agent.start_nodes[path_num],
                                            target=agent.finish_nodes[path_num], 
                                            agent=agent, 
    ...
    def calculate_expected_edge_time_for_edge(self, agent, path_num, edge_num):
        ...
        if edge_num == 0:
            # Expected edge time is calculated as the edge subjective
            # (for agent) weight divided by 60 kmh (default speed)
            # Expected edge time is a time in which agent expects
            # to finish traversing the edge. Expected times are 
            # different for those who use navigator and those who don't.
    ...
    def simulation_iteration_DS(self):
        ...
        while arch_time <= int((self.max_time/self.agents[0].time_stamp)):
            ...
            # Agent finds path and expects to traverse
            # it for some time Arch-loop.
            for a in self.agents:
                for path_num in range(a.current_path, len(a.paths)):
                    ...
                    if (a.times_of_departures[path_num] == t
                        and a.paths[path_num] is None and a.current_edge == (0,0)):
    ...
    # Function that claculates the weighted average difference between
    # the forecast of the method used for current_path and reality.
    ...
    # Function that checks whether every agent sticks to his choice
    # of method (for a given path) for at least 2 iterations.
        ...
        if len(self.common_sense) - self.iteration >= 1:
            ...
            for iteration in range(self.iteration, len(self.common_sense)):
                for a in self.common_sense[iteration]:
                    for p in range(0, len(a.paths)):
                        if (a.path_type[p] 
                            != self.common_sense[iteration-1][a.id_num].path_type[p]):
